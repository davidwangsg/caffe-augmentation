...
void DataTransformer<Dtype>::Transform(const cv::Mat& img,
  ...
  if ( rotation_angle_interval != 1 && phase_ == TRAIN) {
    ...
    if(rotation_angle_interval == 90){
      ...
      if (phase_ == TRAIN) {
        ...
        cv::Mat circle_mask_3c;
        cv::Mat circle_mask(img_rotated.rows, img_rotated.cols, CV_8UC1, cv::Scalar(0, 0, 0));
        cv::circle(circle_mask, cv::Point( img_rotated.rows / 2, img_rotated.cols / 2 ), img_rotated.rows / 2, cv::Scalar( 255, 255, 255), -1, 8 );
        cv::cvtColor(circle_mask,circle_mask_3c,CV_GRAY2BGR);

        cv::bitwise_and(img_rotated, circle_mask_3c, img_rotated_masked);

        img_rotated_masked.copyTo(cv_img);
      } else {
        img_rotated = cv_cropped_img;
        cv::Mat circle_mask_3c;
        cv::Mat circle_mask(img_rotated.rows, img_rotated.cols, CV_8UC1, cv::Scalar(0, 0, 0));
        cv::circle(circle_mask, cv::Point( img_rotated.rows / 2, img_rotated.cols / 2 ), img_rotated.rows / 2, cv::Scalar( 255, 255, 255), -1, 8 );
        cv::cvtColor(circle_mask,circle_mask_3c,CV_GRAY2BGR);
      ...
      if (display && phase_ == TRAIN)
        cv::imshow("Rotation", img_rotated_masked);
  ...
  //LOG(INFO) << "Write Final Image" << rotation_degree;
  //cv::imwrite("a002.png", cv_img);
